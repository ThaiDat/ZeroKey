    ...
    'h':
       begin
         ...
         case LowerCase(st[lengthSt-1]) of
           'n','c':
              if pos(Alphabet_NoSign[GetPosInFull( Alphabet_Sign[GetPosInFull(st[lengthSt-2])] )] , 'aAeEiI' ) = 0 then exit(true);
           'p','t','k': exit(true);
           'g' : if (st[lengthSt-2] <> 'N') and (st[lengthSt-2] <> 'n') then exit(true);
         end;
       end;
    't','p','c','n','m':
       if pos(Alphabet_NoSign[GetPosInFull( Alphabet_Sign[GetPosInFull(st[lengthSt-1])] )] , 'aAeEiIoOuUyY' ) = 0 then exit(true);
  end;
